...
private const val INDEX_RATE = 0.70f
private const val PROTECT = 0.33f
private const val RMS_MIX_RATE = 0.25f
private const val POST_GAIN = 1.18f
private const val LIMITER_CEILING = 0.8912509f // -1 dBFS
...
class RvcPipeline(
    ...
    fun convert(
        ...
        val rmsMatched = if (RMS_MIX_RATE < 1f) {
            changeRms(audio16k, 16_000, synthesized, metadata.samplingRate, RMS_MIX_RATE)
        } else synthesized
        val audio = postGainAndSoftLimit(rmsMatched)
        onProgress(1f)
    ...
    private fun postGainAndSoftLimit(input: FloatArray): FloatArray {
        if (input.isEmpty()) return input
        val ceiling = LIMITER_CEILING
        return FloatArray(input.size) { i ->
            val x = input[i] * POST_GAIN
            // Linear around zero and smoothly bounded at -1 dBFS.
            (ceiling * kotlin.math.tanh((x / ceiling).toDouble())).toFloat()
        }
    }
...
main = main_path.read_text().replace('A26 AI Female Voice v1.2', 'A26 AI Female Voice v1.4')
main_path.write_text(main)

gradle_path = ROOT / 'app/build.gradle.kts'
gradle = gradle_path.read_text().replace('versionName = "1.2-natural"', 'versionName = "1.4-natural"')
gradle_path.write_text(gradle)
print('branded v1.4-natural')
